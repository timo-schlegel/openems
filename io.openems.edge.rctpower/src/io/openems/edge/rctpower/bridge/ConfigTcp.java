package io.openems.edge.rctpower.bridge;

import org.osgi.service.metatype.annotations.AttributeDefinition;
import org.osgi.service.metatype.annotations.ObjectClassDefinition;

import io.openems.edge.rctpower.bridge.api.AbstractRctPowerBridge;
import io.openems.edge.rctpower.bridge.api.LogVerbosity;

@ObjectClassDefinition(//
		name = "Bridge RctPower", //
		description = "Provides a service for connection to, querying and writing to a RCT-Power device.")
@interface ConfigTcp {

	@AttributeDefinition(name = "Component-ID", description = "Unique ID of this Component")
	String id() default "rctpower0";

	@AttributeDefinition(name = "Alias", description = "Human-readable name of this Component; defaults to Component-ID")
	String alias() default "";

	@AttributeDefinition(name = "Is enabled?", description = "Is this Component enabled?")
	boolean enabled() default true;
	
	@AttributeDefinition(name = "IP-Address", description = "The IP address of the Modbus/TCP device.")
	String ip();

	@AttributeDefinition(name = "Port", description = "The port of the Modbus/TCP device.")
	int port() default AbstractRctPowerBridge.DEFAULT_PORT;
	
	@AttributeDefinition(name = "Log-Verbosity", description = "The log verbosity.")
	LogVerbosity logVerbosity() default LogVerbosity.NONE;
	
	@AttributeDefinition(name = "Invalidate elements after how many read Errors?", description = "Increase this value if read errors happen frequently.")
	int invalidateElementsAfterReadErrors() default 1;	

	String webconsole_configurationFactory_nameHint() default "Bridge RctPower [{id}]";

}